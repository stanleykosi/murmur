    ...
    expect(() =>
      module.parseAgentEnvironment(
        createValidEnvironment({
          ...
          AGENT_TURN_DEADLINE_MS: "0",
          SENTRY_DSN: "still-not-a-url",
        }),
      ),
    ).toThrowError(
      /DATABASE_URL|REDIS_URL|OPENROUTER_API_KEY|OPENROUTER_DEFAULT_MAX_TOKENS|OPENROUTER_REQUEST_TIMEOUT_MS|AGENT_TURN_DEADLINE_MS|SENTRY_DSN/,
    );
  });
