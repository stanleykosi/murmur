...
export function createSpeakNode(bindings: AgentGraphBindings) {
  return async function speakNode(
    ...
    try {
      const audioBuffer = await bindings.ttsProvider.synthesize(
        currentResponse,
        bindings.agent.voiceId,
      );

      await bindings.sessionBridge.speakText(currentResponse, audioBuffer);
      ...
      const transcriptEvent: TranscriptEvent = {
        ...
        wasFiltered: state.currentResponseWasFiltered,
      };
